...
async def current_product_allocation_state_change_user(user=Depends(current_active_user)) -> auth_model.User:
    user_rules = get_rules_from_user(user)
    if any([common.RuleName.IMS_DEV in user_rules,
            common.RuleName.ADMIN in user_rules]):
        return user

    else:
        raise exceptions.InsufficientPrivilegeError('User {user.name} does not have privilege'
                                                    ' to change production allocation state')
